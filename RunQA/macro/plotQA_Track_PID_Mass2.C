...
void plotQA_Track_PID_Mass2(int energy = 0, string JobId = "028AC6C12A1F110244470E4CCDFC40FF")
{
  ...
  for(int i_par = 0; i_par < 4; ++i_par)
  {
    ...
    {
      std::string HistName = Form("h_mMass2%s%s_trigger9",mParticle[i_par].c_str(),mCutsQA[i_cut].c_str());
      h_mMass2[i_par][i_cut] = (TH1F*)File_InPut->Get(HistName.c_str());
    }
  }
  ...
  std::string outputname = Form("./figures/%s/c_TrackQA_PID_Mass2_%s_%s.pdf",runQA::mBeamEnergy[energy].c_str(),runQA::mBeamEnergy[energy].c_str(),JobId.c_str());
