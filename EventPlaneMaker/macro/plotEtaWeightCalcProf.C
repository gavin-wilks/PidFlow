...
void plotEtaWeightCalcProf(int beamEnergy = 1)
{
  ...
  string inputfileEpd = Form("../StRoot/StEventPlaneUtility/EpdResolution/file_%s_EpdResEta.root",recoEP::mBeamEnergy[beamEnergy].c_str());
  string inputfileTpc = Form("../StRoot/StEventPlaneUtility/Resolution/file_%s_Resolution.root",recoEP::mBeamEnergy[beamEnergy].c_str());
  ...
  for(int order = 1; order <= 3; ++order)
  {
    ...
    { 
      ...
      cout << "EPD: order " << order << "  cent " << i_cent << "   Res Raw = " << resRaw << "   Err = " << errRaw << endl;
      if(resRaw > 0) 
      { 
...
*/
  std::string outputname = Form("./figures/EtaWeightsv2_%s.pdf",recoEP::mBeamEnergy[beamEnergy].c_str());
  ...
  for(int order = 1; order <= 3; order++)
  {
    for(int i_cent = 0; i_cent < 9; i_cent++)
    {
      ...
      fit[order-1][i_cent] = new TF1(Form("f%d%d",order,i_cent),"[0]*x+[1]*x*x*x",-5.5,5.5);
