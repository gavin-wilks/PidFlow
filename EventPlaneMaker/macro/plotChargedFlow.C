...
void plotChargedFlow(int beamEnergy = 1, int cent9 = 9)
{
  ...
  for(int i_cent = 0; i_cent < 10; ++i_cent)
  {
    ...
    p_mChargedV1EpQA[i_cent] = (TProfile*)File_InPut->Get(ProName.c_str()); 
    ...
    p_mChargedV1Ep[i_cent] = (TProfile*)File_InPut->Get(ProName.c_str());
  ...
  string FigureName = Form("./figures/c_mChargedV1Ep_%s.pdf",recoEP::mBeamEnergy[beamEnergy].c_str());
  ...
  p_mChargedV1Ep[cent9]->SetMarkerStyle(24);
  p_mChargedV1Ep[cent9]->SetMarkerColor(kAzure+2);
  p_mChargedV1Ep[cent9]->SetMarkerSize(1.5);
  p_mChargedV1Ep[cent9]->SetLineColor(kAzure+2);
  p_mChargedV1Ep[cent9]->Draw("pE1X0 same");

  TLegend *leg = new TLegend(0.20,0.70,0.45,0.85);
  leg->SetFillColor(10);
  leg->SetBorderSize(0);
  leg->AddEntry(p_mChargedV1Ep[cent9],"v_{1}^{EP,TPC}","p");
  leg->Draw("same");
  FigureName = Form("./figures/c_mChargedV1Ep_%s_Cent%d.pdf",recoEP::mBeamEnergy[beamEnergy].c_str(),cent9);
  ...
  FigureName = Form("./figures/c_mChargedV3Ep_%s_Cent%d.pdf",recoEP::mBeamEnergy[beamEnergy].c_str(),cent9);
  c_v3->SaveAs(FigureName.c_str());

  TCanvas *c_QA = new TCanvas("c_QA","c_QA",10,10,1200,900);
  c_QA->Divide(1,3);
  ...
  p_mChargedV1EpQA[cent9]->SetMarkerStyle(24);
  p_mChargedV1EpQA[cent9]->SetMarkerColor(kGray+2);
  p_mChargedV1EpQA[cent9]->SetMarkerSize(0.5);
  p_mChargedV1EpQA[cent9]->SetLineColor(kGray+2);
  p_mChargedV1EpQA[cent9]->Draw("pE1X0 same");


  c_QA->cd(2); // <v2Ep> vs. runIndex
